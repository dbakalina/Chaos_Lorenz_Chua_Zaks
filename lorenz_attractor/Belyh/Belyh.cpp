...
void Belyh_model(double* point, double* dPoint, double* parameters) {
    //Обозначения:
    //point={x,y,z}
    //dPoint={dx/dt, dy/dt, dz/dt}
    //parameters={alpha(0), beta(1), lambda(2)}
    ...
    dPoint[0] = point[1];
    dPoint[1] = -(point[0] * point[0] + point[2] - 1) * point[0] - parameters[2] * point[1];
    dPoint[2] = -parameters[0] * point[2] + parameters[1] * point[0] * point[0];
...
int main()
{
    ...
    //parameters={alpha(0), beta(1), mu(2), delta(3)}
           //alpha
    parameters[1] = 2;       //beta
    parameters[2] = 1;  //lambda
    //parameters[0] = (-parameters[2] + sqrt(parameters[2] * parameters[2] + 4)) / 2 + 0.01;
    parameters[0] = 1;
    ...
    cout << "lambda = " << parameters[2] << endl;         //mu
    //cout << "delta = " << parameters[3] << endl;      //delta

    f3.open("point_Belyh_model.txt", ios::out);
    ...
    point[0] = 0.01;
    point[1] = 0.01;    //point[1] = ((parameters[0] + lambda) / parameters[0]) * 0.001;
    point[2] = 0;
   ...
   // f1.open("equilibrium_Arneodo.txt", ios::out);
    ...
    for (int i = 0; i < time; i++) {
        RK(Belyh_model, point, dPoint, parameters, &dim, &dt);
        if (i > 16000) {
